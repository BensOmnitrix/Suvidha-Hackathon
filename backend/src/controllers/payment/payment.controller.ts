 ...
 */
function extractPaymentDetails(payment: RazorpayPayment): PaymentDetails {
...
export const verifyPayment = asyncHandler(
  async (
    req: Request<unknown, unknown, VerifyPaymentInput>,
    res: Response<VerifyPaymentResponse | { error: string }>,
  ) => {
    ...
    const result = await prisma.$transaction(async (tx: TransactionClient) => {
      ...
      const payment = await tx.payment.create({
        data: {
          ...
          gatewayResponse: paymentDetails as object,
